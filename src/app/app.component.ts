...
export class AppComponent {
	...
	downloadPdfJs(completeData: MonthData[], pdfType: string) {
		...
		for (let i = 0; i < completeData.length; i++) {
			doc.setFontSize(15);
			doc.text(completeData[i].month_name, pageWidth / 2, 10, { align: 'center' });
			doc.setFontSize(12);
			doc.text("Maintenance", 14, 17);
			autoTable(doc, {
				startY: 20,
				head: [['Flat', 'Owner Name', 'Amount', 'Status', 'Date']],
				body: completeData[i].maintenance.map(row => Object.values(row)).concat([['$', 'Total', completeData[i].savings.maintenance_total]]),
				willDrawCell: (data) => {
				...
				didParseCell: (data) => {
					if (data.row.index === completeData[i].maintenance.length) {
			...
			if (pdfType == 'full') {
				...
				if (completeData[i].earnings.length > 0) {
					doc.text("Earnings", 14, (doc as any).lastAutoTable.finalY + 10);
					autoTable(doc, {
						startY: (doc as any).lastAutoTable.finalY + 12.5,
						head: [['Work', 'Amount', 'Date']],
						body: completeData[i].earnings.map(row => Object.values(row)).concat([['Total', completeData[i].savings.earnings_total]]),
						didParseCell: (data) => {
							if (data.row.index == completeData[i].earnings.length) {
								data.cell.styles.fontStyle = 'bold';
								data.cell.styles.textColor = 'white';
								data.cell.styles.fillColor = '#2969ff';
							}
				...
				if (completeData[i].expenses.length > 0) {
					doc.text("Expenses", 14, (doc as any).lastAutoTable.finalY + 10);
					autoTable(doc, {
						startY: (doc as any).lastAutoTable.finalY + 12.5,
						head: [['Work', 'Amount', 'Date']],
						body: completeData[i].expenses.map(row => Object.values(row)).concat([['Total', completeData[i].savings.expenses_total]]),
						didParseCell: (data) => {
							if (data.row.index === completeData[i].expenses.length) {
								data.cell.styles.fontStyle = 'bold';
								data.cell.styles.textColor = 'white';
								data.cell.styles.fillColor = '#2969ff';
							}
				...
				doc.text("Savings", 14, (doc as any).lastAutoTable.finalY + 10);
				autoTable(doc, {
					startY: (doc as any).lastAutoTable.finalY + 12.5,
					styles: {
						fontStyle: 'bold',
